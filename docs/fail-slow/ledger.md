...
- 具体某个 case 的详细手册在 [`cases/<case_id>.md`](cases/)；本台账只放**速览**，详情指过去，别在这里重抄。
...
|---|---|---|
| mohe-241 | `results/muxi-mohe/<run_id>/` | `/afs-a3-weight-share/yinjinrun.p/results/muxi-mohe/<run_id>/` |
| h3c-test | `results/muxi-h3c/<run_id>/` | 同上前缀 |
...
| 检测工具 | 无 / Probing / Greyhound / XPUTimer / StragglerAnalysis / SuperBench |
...
> 剂量配方真相源：`scripts/fail-slow/dose_recipes.yaml`。详情见各 `cases/<id>.md`。

| Case | 名称 | 注入 kind | 模式 | Loud 实测 C1/C0 | 到达 D | 关键 run_id / 备注 |
|---|---|---|---|---|---|---|
| **P3-EXT-A** | host CPU 争用 | `stress_cpu` | host_bound | **~2.97** | **D4** ✅ | `20260724_090823-p3-live-d4e`；证据 `host_psi_cpu` |
...
| 工具 | 状态 | 原因 |
|---|---|---|
| Greyhound | `ENV-BLOCKED` | NCCL/Redis/Docker 依赖，MetaX 不兼容（记 N/A，非 D0） |
| XPUTimer | `ENV-BLOCKED` | NCCL≤2.21.5 / Bazel，MetaX 不兼容 |
| StragglerAnalysis / SuperBench | 离线，喂 B run trace | 转换脚本待写 |
...
| 跨 rank 联邦查询 | ⚠️ **待核实**：`sop.md` §3.8 称“无内置联邦”，兼容性评审称现已支持 `global.*`——用前对当前 wheel 实测确认 |
...
| 完整长规程 | [`sop.md`](sop.md) |
| 每个 case 详细手册 | [`cases/<id>.md`](cases/) |
