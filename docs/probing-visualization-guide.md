...
PROBING=1 PROBING_PORT=8788 PROBING_ASSETS_ROOT=probing/web/dist \
  ...
  torchrun --nproc_per_node=4 scripts/pretrain_gpt_probing.py ... --train-iters 80
```

> 4 卡 Megatron 请用物理顺序 `CUDA_VISIBLE_DEVICES=0,1,2,3`（配合 `NCCL_SOCKET_IFNAME=lo`）；若 remap 为 `1,2,3,0` 可能导致 NCCL 连错网卡。DDP 热力图 demo 仍可用 `1,2,3,0` 避开 GPU0 占用。

**Dashboard**（4 卡 `torchrun` / `pt_elastic` 线程，preset `gpt126m_4dp`）：

![Megatron Dashboard](./assets/latest/web_megatron_dashboard.png)

**Training 页**（`pretrain_gpt_probing.py` 已挂 phase hook，可看到 step 级热力图）：

![Megatron Training](./assets/latest/web_megatron_training.png)

**Distributed Spans**（Megatron 4 卡 `pretrain_gpt_probing.py`；需在训练仍在运行时打开 `/spans`）：

![Megatron Spans](./assets/latest/web_megatron_spans.png)

采集脚本通过 `scripts/pretrain_gpt_probing.py` 在 `setup_model_and_optimizer` 后 patch Megatron 的 `forward_step` / `backward_step` / `optimizer.step`，写入 `forward` / `backward` / `optimizer` / `train.step` 层级 span（见 `/spans`）。若直接用上游 `pretrain_gpt.py` 而不挂 patch，Spans 页会为空。
...
| `scripts/pretrain_gpt_probing.py` | Megatron 入口：自动 `attach_training_phases` |
