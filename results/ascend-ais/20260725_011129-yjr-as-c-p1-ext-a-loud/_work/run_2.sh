...
export PROBING=2; unset PROBING_TORCH_PROFILING; export PROBING_GPU=on; export PROBING_GPU_BACKEND=npu; export PROBING_NPU_SOURCE=auto; export PROBING_GPU_SAMPLE_MS=1000; export PROBING_CPU=on; export PROBING_CPU_SAMPLE_MS=1000; export PYTHONPATH=/data/yinjinrun.p-huawei/probe-bundle/pydeps:${PYTHONPATH:-}; export PATH=/data/yinjinrun.p-huawei/probe-bundle/pydeps/bin:/root/miniconda3/envs/llm_test/bin:${PATH}; export INLINE_INJECT=cube; export INLINE_VICTIM_LOCAL_RANK=7; export INLINE_INJECT_START=100; export INLINE_INJECT_STOP=300; export INLINE_CUBE_SIZE=8192; export INLINE_CUBE_MM=64;
OUT='/data/yinjinrun.p-huawei/results/ascend-ais/20260725_011129-yjr-as-c-p1-ext-a-loud/P1-EXT-A/by_pod/yysong-master-0/round_1/C2_probing'
...
/root/miniconda3/envs/llm_test/bin/torchrun --nnodes=1 --nproc_per_node=16 --node_rank=0 \
  ...
  --run-id=20260725_011129-yjr-as-c-p1-ext-a-loud --group=2 --config='C2_probing' --round=1 \
