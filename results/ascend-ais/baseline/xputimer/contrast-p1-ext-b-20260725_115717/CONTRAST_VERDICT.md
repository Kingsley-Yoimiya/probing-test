# XPUTimer contrast · P1-EXT-B Loud

- case_id: `P1-EXT-B`
- case_ref: `20260725_014350-yjr-as-c-p1-ext-b-loud`
- dose: INLINE hbm mb=512 copies=48 victim=7; window [100,300]
- detect_mode: **cross_run_contrast** （自主=prom hang/slow flags；跨-run=C1/C0 中位比≥1.6，需外部健康基线 C0）
...
**C1/C0 coll ratio = 0.982** → FAIL (thr 1.6)

> ⚠️ `≥1.5×C0med` 计数仅作噪声诊断：C0 健康线自身就有 2056 个，说明该线在集合通信 host-wall 上可能大面积误报，**不作判据**。

## C) dose_check（step_ms 窗内中位；非 XPUTimer 规则）

- window: [100, 300)
- C0 median step_ms: 75.502 (n=3200)
- C1 median step_ms: 156.208 (n=3200)
- C1/C0 step_ms = 2.069 → PASS (thr 1.6)

## Verdict

- **autonomous_detect**: NO (XPUTimer 自己的 hang/slow flags)
- **cross_run_contrast**: FAIL (C1/C0=0.982；需外部基线)
- **dose_check**: PASS (step_ms C1/C0=2.069)
