# XPUTimer contrast · P1-HW-B Loud

- case_id: `P1-HW-B`
- case_ref: `20260725_142359-yjr-as-c-p1-hw-b-loud`
- dose: INLINE hbm ramp mb=512 copies=6→48 victim=7; window [100,300]; gold≈1.57
...
> ⚠️ `≥1.5×C0med` 计数仅作噪声诊断：C0 健康线自身就有 1535 个，说明该线在集合通信 host-wall 上可能大面积误报，**不作判据**。

## C) dose_check（step_ms 窗内中位；非 XPUTimer 规则）

- window: [100, 300)
- C0 median step_ms: 75.947 (n=3200)
- C1 median step_ms: 120.346 (n=3200)
- C1/C0 step_ms = 1.585 → PASS (thr 1.3)

## Verdict

- **autonomous_detect**: NO (XPUTimer 自己的 hang/slow flags)
- **cross_run_contrast**: FAIL (C1/C0=0.991；需外部基线)
- **dose_check**: PASS (step_ms C1/C0=1.585)
