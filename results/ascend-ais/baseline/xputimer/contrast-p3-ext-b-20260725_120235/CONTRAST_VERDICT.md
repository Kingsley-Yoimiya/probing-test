# XPUTimer contrast · P3-EXT-B Loud

- case_id: `P3-EXT-B`
- case_ref: `20260725_020212-yjr-as-c-p3-ext-b-loud`
- dose: stress_io fio_nj=16,iodepth=64,bs=4k,size=4G,ckpt_every=20,io_read_kb=1024; window [100,300]
...
|-----|--:|-------------:|------------------------------:|
| C0  | 70336 | 124.0 | 10603 |
| C1  | 70336 | 130.0 | 13637 |

**C1/C0 coll ratio = 1.048** → FAIL (thr 1.3)

> ⚠️ `≥1.5×C0med` 计数仅作噪声诊断：C0 健康线自身就有 10603 个，说明该线在集合通信 host-wall 上可能大面积误报，**不作判据**。

## C) dose_check（step_ms 窗内中位；非 XPUTimer 规则）

- window: [100, 300)
- C0 median step_ms: 173.704 (n=3200)
- C1 median step_ms: 311.403 (n=3200)
- C1/C0 step_ms = 1.793 → PASS (thr 1.3)

## Verdict

- **autonomous_detect**: NO (XPUTimer 自己的 hang/slow flags)
- **cross_run_contrast**: FAIL (C1/C0=1.048；需外部基线)
- **dose_check**: PASS (step_ms C1/C0=1.793)
