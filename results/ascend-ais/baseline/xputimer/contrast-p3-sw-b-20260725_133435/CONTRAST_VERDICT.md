# XPUTimer contrast · P3-SW-B Loud

- case_id: `P3-SW-B`
- case_ref: `20260725_125558-yjr-as-c-p3-sw-b-loud`
- dose: INLINE 8b mb=16 stall_s=0.25 victim=7; window [100,300]
...
|-----|--:|-------------:|------------------------------:|
| C0  | 70336 | 126.0 | 10950 |
| C1  | 70336 | 125.0 | 9300 |

**C1/C0 coll ratio = 0.992** → FAIL (thr 1.3)

> ⚠️ `≥1.5×C0med` 计数仅作噪声诊断：C0 健康线自身就有 10950 个，说明该线在集合通信 host-wall 上可能大面积误报，**不作判据**。

## C) dose_check（step_ms 窗内中位；非 XPUTimer 规则）

- window: [100, 300)
- C0 median step_ms: 159.811 (n=3200)
- C1 median step_ms: 327.118 (n=3200)
- C1/C0 step_ms = 2.047 → PASS (thr 1.3)

## Verdict

- **autonomous_detect**: NO (XPUTimer 自己的 hang/slow flags)
- **cross_run_contrast**: FAIL (C1/C0=0.992；需外部基线)
- **dose_check**: PASS (step_ms C1/C0=2.047)
