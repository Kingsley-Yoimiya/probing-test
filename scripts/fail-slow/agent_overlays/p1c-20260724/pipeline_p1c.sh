...
start_sidecar() {   # 在 victim(node0)起注入; freq / 内联 8a 不走这里
  ...
  case "$INJECT_KIND" in
    ...
    8a|inline_8a|2c|none) : ;;  # 8a/2c 走训练进程 INLINE_INJECT
...
wait_sidecar_start() {  # $1=out_dir；GPU sidecar 必须见到 SIDECAR_START，否则注入窗空转
  ...
  while [ "$e" -lt "$budget" ]; do
    ...
    # 进程已死且无 START → 失败，勿空等（匹配 sidecar_inject.py / sidecar_inject_v2.py）
...
is_gpu_sidecar() {
  case "$INJECT_KIND" in
    cube|hbm|1b|1c|2b|3c|5b) return 0 ;;  # 2c 改 INLINE，不再走 GPU sidecar 门禁
    *) return 1 ;;
  esac
}
stop_sidecar() {
...
wait_done() {   # $1=out_dir $2=是否按 stop marker 停 sidecar
  ...
  # overlay：MetaX 上 torchrun 拆连接后常不写 done/fail → 用 jsonl 完整度兜底，避免空等 900s
  ...
  while [ $e -lt 360 ]; do
    ...
    if [ "${LOCAL_FS:-0}" = "1" ]; then
      ...
      while [ "$n" -lt "$NNODES" ]; do
        if pexec "${PODS[$n]}" "test -f '$out/node_${n}.done'" >/dev/null 2>&1; then
          ...
          # 兜底：训练已写满 jsonl 但 launcher 未落 done（teardown 挂死）→ 补 done
          if [ "$e" -ge 60 ]; then
            if pexec "${PODS[$n]}" "n=\$(wc -l < '$out/ranks/rank_'*'.jsonl' 2>/dev/null | awk '{s+=\$1}END{print s+0}'); [ \"\${n:-0}\" -ge $((need_steps * NPROC / 2)) ] && (pkill -9 -f '[t]orchrun' 2>/dev/null||true; pkill -9 -f '/tmp/[t]bp.py' 2>/dev/null||true; touch '$out/node_${n}.done')" >/dev/null 2>&1; then
              d=$((d + 1))
              echo "  heal: marked node_${n}.done (jsonl complete)"
            elif [ "$e" -ge 120 ]; then
              # 再等一会仍无产物 → 标 fail 以免死等
              pexec "${PODS[$n]}" "pkill -9 -f '[t]orchrun' 2>/dev/null||true; pkill -9 -f '/tmp/[t]bp.py' 2>/dev/null||true; echo 124 > '$out/node_${n}.fail'" >/dev/null 2>&1 || true
              f=$((f + 1))
              echo "  heal: marked node_${n}.fail (no progress)"
            fi
          fi
        fi
        n=$((n + 1))
    ...
    # 全部节点有终态（done+fail）即可结束；有 fail 则 rc=1
    if [ $((d + f)) -ge "$NNODES" ] 2>/dev/null; then
      if [ "${f:-0}" != "0" ] && [ "${f:-0}" != "" ]; then
        echo "  FAIL marker seen (f=$f d=$d)"
        return 1
      fi
      echo "  done markers $d/$NNODES (${e}s)"
      return 0
    ...
    if [ $((e % 30)) -eq 0 ]; then
      echo "  waiting done… d=${d:-0}/$NNODES f=${f:-0} t=${e}s"
    fi
    sleep 5; e=$((e+5))
  done
...
export INLINE_GC_STALL_S=${INLINE_GC_STALL_S:-0.25};"
    ...
    # P1-SW-C overlay：INLINE_INJECT=2c（编译 tip 必须在训练进程内）
    if [ "$inj" = "yes" ] && [ "$INJECT_KIND" = "2c" ]; then
      denv="${denv}
...
export INLINE_2C_FALLBACK_S=${INLINE_2C_FALLBACK_S:-0.2};
export TORCHINDUCTOR_CACHE_DIR=/tmp/p1c_inductor_cache;"
    fi
    # Greyhound collect-min dump 路径（与 p1swa pipeline_local 对齐）
...
export INLINE_HBM_COPIES=${INLINE_HBM_COPIES:-48};"
    ...
    # 内联 8a / inline hbm / inline 2c 无需外部 stop
