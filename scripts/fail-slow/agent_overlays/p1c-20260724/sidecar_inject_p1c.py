...
def case_2c(seconds: float):
    """编译缓存 thrash: 周期性清 torch inductor cache 强制 recompile（sidecar 近似；正式 2C 走 INLINE）"""
    import torch
    torch.cuda.set_device(0)
    print("SIDECAR_WARMUP kind=2c", flush=True)
    # 预热：先起 CUDA context，避免 MetaX 时间片假阴性
    _w = torch.randn(512, 512, device="cuda:0")
    torch.mm(_w, _w)
    torch.cuda.synchronize()
    del _w
    print("SIDECAR_START kind=2c", flush=True)
    import shutil
    cache_dir = os.environ.get("TORCHINDUCTOR_CACHE_DIR", os.path.expanduser("~/.cache/torch_inductor"))
    t_end = time.time() + seconds
    while time.time() < t_end:
...
def case_3c(seconds: float):
    ...
    torch.cuda.set_device(0)
    print("SIDECAR_WARMUP kind=3c", flush=True)
    _w = torch.randn(1024, 1024, device="cuda:0", dtype=torch.float16)
    torch.mm(_w, _w)
    torch.cuda.synchronize()
    del _w
