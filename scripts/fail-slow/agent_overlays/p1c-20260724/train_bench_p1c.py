...
def main() -> None:
    ...
    def step_instrumented(
        ...
        # MetaX 上 compile 可能失败；失败则 sleep 近似尖刺，保证 Loud 可咬且不拖垮 MCCL。
        if compile_spike_n > 0:
            import shutil as _shutil
            _cache = os.environ.get(
                "TORCHINDUCTOR_CACHE_DIR",
                "/tmp/p1c_inductor_cache",
            )
            try:
    ...
    # P1-SW-C overlay：INLINE_INJECT=2c → victim 在注入窗内制造 compile one-shot 尖刺
    do_inline_2c = (
        inline == "2c"
        and local == inline_victim
        and node_rank == 0
    ...
    if do_inline_2c:
        print(
            ...
            f"fallback_s={os.environ.get('INLINE_2C_FALLBACK_S', '0.2')} "
            f"win=[{inline_start},{inline_stop})",
            flush=True,
        )
    leak_buf: list = []
    hbm_bufs = None
