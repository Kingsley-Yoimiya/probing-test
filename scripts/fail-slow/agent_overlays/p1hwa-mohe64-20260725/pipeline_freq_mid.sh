...
#   - P1-HW-A overlay: 中途降频（warmup+measure start 后）victim-only xcore；结束恢复
...
# freq 分支: victim-only（OUTLINE 1A 目标卡）；需 remount rw /sys
freq_set() {   # $1=level
  local lvl="$1" vr="${FREQ_VICTIM_ID:-$SIDECAR_LOCAL_RANK}"
  pexec "$MASTER" "mount -o remount,rw /sys 2>/dev/null; mx-smi -i $vr --set-dpm-max xcore,$lvl >/dev/null 2>&1; echo FREQ_SET_victim${vr}_$lvl; mx-smi -i $vr --show-dpm-max 2>/dev/null | grep -E 'xcore|GPU#' | head -4" 2>/dev/null
}
freq_restore() {
  local vr="${FREQ_VICTIM_ID:-$SIDECAR_LOCAL_RANK}"
  pexec "$MASTER" "mount -o remount,rw /sys 2>/dev/null; mx-smi -i $vr --set-dpm-max xcore,9 >/dev/null 2>&1; echo FREQ_RESTORED_victim${vr}" 2>/dev/null
}
...
export INLINE_HBM_COPIES=${INLINE_HBM_COPIES:-48};"
    ...
    if [ "$IS_FREQ" = "1" ] && [ "$inj" = "yes" ]; then
      # P1-HW-A / OUTLINE 1A：先满频起训，measure start 后中途降 victim xcore
      freq_restore || true
      fire_training "$port" "$out" "$denv" "$r"; echo "  fired(freq-mid-armed level=$FREQ_LEVEL)"
      wait_warmup "$out"
      if wait_measure_step "$out" "$INJECT_START_MEASURE_STEP"; then
        freq_set "$FREQ_LEVEL"
        pexec "$MASTER" "printf '%s\n' \
          'SIDECAR_WARMUP kind=freq mid_after_measure_${INJECT_START_MEASURE_STEP}' \
          'SIDECAR_START kind=freq level=${FREQ_LEVEL} victim=${SIDECAR_LOCAL_RANK}' \
          'FREQ_MID_SET level=${FREQ_LEVEL} victim=${SIDECAR_LOCAL_RANK}' \
          >'$out/injection.log'" 2>/dev/null || true
        echo "  freq MID-SET level=$FREQ_LEVEL victim=$SIDECAR_LOCAL_RANK"
      else
        echo "  FAILED: freq mid inject skipped (no start marker)"
        pipe_rc=1
      fi
    else
