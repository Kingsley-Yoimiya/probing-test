...
case "$CASE_ID" in
  ...
  P3-SW-B|8b)
    ...
    ;;
  P1-HW-A|1a)
    # OUTLINE 1A cube-proxy 热墙代理（freq level<=2 仍不足时）
    CASE="P1-HW-A"; INJECT_KIND="cube"
    INJECT_ARGS="${INJECT_ARGS:-duty=0.95,size=8192}"
    MODE="${MODE:-gpu_bound}"
    ACCEPT_MIN_RATIO="${ACCEPT_MIN_RATIO:-1.3}"
    SIDECAR_WARMUP="${SIDECAR_WARMUP:-45}"
    export SIDECAR_WARMUP
    ;;
  *) echo "unsupported CASE_ID=$CASE_ID (P1-HW-A/…)" >&2; exit 2 ;;
esac
...
run_config() {
  ...
  CASE="$CASE" INJECT_KIND="$INJECT_KIND" INJECT_ARGS="$INJECT_ARGS" \
    ...
    bash "${PIPELINE_SH:-$HERE/pipeline_freq_mid.sh}"
}
