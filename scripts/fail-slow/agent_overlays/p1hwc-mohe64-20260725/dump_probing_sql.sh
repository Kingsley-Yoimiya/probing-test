...
collect_host_gpu() {
  ...
  # P1-HW-A/C 旁证：同窗温/频/功率 + dpm-max（真改频 / 间歇脉冲）
  if [ "$CASE" = "P1-HW-A" ] || [ "$CASE" = "P1-HW-C" ]; then
    "$mx" -i "$dev" --show-dpm-max >"$DUMP/host_gpu_dpm_max.txt" 2>&1 || true
    "$mx" -i "$dev" --show-dpm cur >"$DUMP/host_gpu_dpm_cur.txt" 2>&1 || true
    "$mx" -i "$dev" --show-clocks xcore >"$DUMP/host_gpu_clocks_xcore.txt" 2>&1 || true
    "$mx" -i "$dev" --show-temperature >"$DUMP/host_gpu_temperature.txt" 2>&1 || true
    "$mx" -i "$dev" --show-board-power >"$DUMP/host_gpu_board_power.txt" 2>&1 || true
    "$mx" -i "$dev" --show-pmbus-power >"$DUMP/host_gpu_pmbus_power.txt" 2>&1 || true
  fi
  python3 - <<PY
import json, re
from pathlib import Path
...
elif case in ("P1-HW-A", "P1-HW-C"):
    dpm = (dump / "host_gpu_dpm_max.txt").read_text(errors="ignore") if (dump / "host_gpu_dpm_max.txt").exists() else ""
    clk = (dump / "host_gpu_clocks_xcore.txt").read_text(errors="ignore") if (dump / "host_gpu_clocks_xcore.txt").exists() else ""
    temp = (dump / "host_gpu_temperature.txt").read_text(errors="ignore") if (dump / "host_gpu_temperature.txt").exists() else ""
    power = (dump / "host_gpu_board_power.txt").read_text(errors="ignore") if (dump / "host_gpu_board_power.txt").exists() else ""
    m_xcore = re.search(r"xcore\s*:\s*(\d+)", dpm, re.I)
    xcore_dpm = int(m_xcore.group(1)) if m_xcore else None
    m_mhz = re.search(r"(\d+)\s*MHz", clk)
    xcore_mhz = int(m_mhz.group(1)) if m_mhz else None
    m_temp = re.search(r"(\d+(?:\.\d+)?)\s*C", temp)
    temp_c = float(m_temp.group(1)) if m_temp else None
    m_pwr = re.search(r"(\d+(?:\.\d+)?)\s*W", power)
    power_w = float(m_pwr.group(1)) if m_pwr else None
    hit = xcore_dpm is not None and xcore_dpm <= 1
    evidence = "host_mx_smi_dpm_freq" if hit else "host_mx_smi_dpm_no_hit"
else:
    hit = False
    evidence = "host_mx_smi_unused"
    xcore_dpm = xcore_mhz = temp_c = power_w = None
...
if case in ("P1-HW-A", "P1-HW-C"):
    blob.update({
        "xcore_dpm_max": xcore_dpm,
        "xcore_mhz": xcore_mhz,
        "temp_c": temp_c,
        "board_power_w": power_w,
    })
    if case == "P1-HW-C":
        blob["note"] = "freq_pulse intermittent; single-shot dump may land on PULSE_HIGH"
