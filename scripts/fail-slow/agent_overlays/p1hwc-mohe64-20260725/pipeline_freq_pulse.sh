...
#   - P1-HW-C overlay: measure 窗内间歇脉冲 victim-only dpm（低档短打→满档恢复）；结束恢复
...
SEQ="${SEQ:-1024}"
BATCH="${BATCH:-8}"
FREQ_LEVEL="${FREQ_LEVEL:-0}"          # pulse 低档；OUTLINE 1C 间歇打低
FREQ_DOMAIN="${FREQ_DOMAIN:-xcore}"    # xcore|mc；campaign 亦可 level=mc
FREQ_RESTORE_LEVEL="${FREQ_RESTORE_LEVEL:-9}"  # xcore 满档=9；mc 满档=3
PULSE_ON_S="${PULSE_ON_S:-2}"          # 低档持续秒
PULSE_OFF_S="${PULSE_OFF_S:-8}"        # 满档间隔秒（占空比低 → 中位≈1）
...
# mc 默认满档 3（见 mx-smi --show-dpm-max）
if [ "$FREQ_DOMAIN" = "mc" ] && [ "${FREQ_RESTORE_LEVEL}" = "9" ]; then
  FREQ_RESTORE_LEVEL=3
fi
...
if [ "${USE_VCCTL:-0}" = "1" ]; then
  ...
  KT="${KUBECTL_REQUEST_TIMEOUT:-60s}"
  pexec()   { kubectl --kubeconfig="$KC" -n "$NS" --request-timeout="$KT" exec    "$1" -- bash -c "$2"; }
  pexec_i() { kubectl --kubeconfig="$KC" -n "$NS" --request-timeout="$KT" exec -i "$1" -- bash -c "$2"; }
  pod_ip()  { kubectl --kubeconfig="$KC" -n "$NS" --request-timeout="$KT" get pod "$1" -o jsonpath='{.status.podIP}' 2>/dev/null; }
fi
...
# freq/pulse 分支: victim-only（OUTLINE 1C 间歇）；需 remount rw /sys
freq_set() {   # $1=level
  local lvl="$1" vr="${FREQ_VICTIM_ID:-$SIDECAR_LOCAL_RANK}" dom="${FREQ_DOMAIN:-xcore}"
  pexec "$MASTER" "mount -o remount,rw /sys 2>/dev/null; mx-smi -i $vr --set-dpm-max ${dom},$lvl >/dev/null 2>&1; echo FREQ_SET_victim${vr}_${dom}_$lvl; mx-smi -i $vr --show-dpm-max 2>/dev/null | grep -E '${dom}|xcore|mc|GPU#' | head -6" 2>/dev/null
}
freq_restore() {
  local vr="${FREQ_VICTIM_ID:-$SIDECAR_LOCAL_RANK}" dom="${FREQ_DOMAIN:-xcore}" rl="${FREQ_RESTORE_LEVEL:-9}"
  pexec "$MASTER" "mount -o remount,rw /sys 2>/dev/null; mx-smi -i $vr --set-dpm-max ${dom},$rl >/dev/null 2>&1; echo FREQ_RESTORED_victim${vr}_${dom}_$rl" 2>/dev/null || true
}

# 在 master 上起后台脉冲：低档 PULSE_ON_S → 满档 PULSE_OFF_S，直到 stop 文件或超时
start_freq_pulse() {
  local out="$1"
  local vr="${FREQ_VICTIM_ID:-$SIDECAR_LOCAL_RANK}"
  local dom="${FREQ_DOMAIN:-xcore}"
  local lo="${FREQ_LEVEL:-0}"
  local hi="${FREQ_RESTORE_LEVEL:-9}"
  local on_s="${PULSE_ON_S:-2}"
  local off_s="${PULSE_OFF_S:-8}"
  local stopf="$out/pulse.stop"
  # 先杀净残留脉冲（双实例会把 xcore 0↔9 打成 SIGABRT）
  pexec "$MASTER" "touch '$out/pulse.stop' 2>/dev/null; pkill -9 -f '/tmp/freq_pulse.sh' 2>/dev/null || true; sleep 1; rm -f '$out/pulse.stop' '$out/pulse.pid'; mx-smi -i $vr --set-dpm-max ${dom},${hi} >/dev/null 2>&1 || true; exit 0" 2>/dev/null || true
  # 把脉冲脚本落到 master，避免复杂嵌套引号；flock 保证单实例
  pexec "$MASTER" "mkdir -p '$out'; rm -f '$stopf'; cat > /tmp/freq_pulse.sh <<'PSEOF'
#!/usr/bin/env bash
set -u
OUT=\"\$1\"; VR=\"\$2\"; DOM=\"\$3\"; LO=\"\$4\"; HI=\"\$5\"; ON=\"\$6\"; OFF=\"\$7\"
STOP=\"\$OUT/pulse.stop\"; LOG=\"\$OUT/injection.log\"; LOCK=\"\$OUT/pulse.lock\"; n=0
exec 9>\"\$LOCK\"
if ! flock -n 9; then echo \"PULSE_SKIP already_locked\" >>\"\$LOG\"; exit 0; fi
echo \$\$ >\"\$OUT/pulse.pid\"
mount -o remount,rw /sys 2>/dev/null || true
while [ ! -f \"\$STOP\" ] && [ \$n -lt 120 ]; do
  mx-smi -i \"\$VR\" --set-dpm-max \"\$DOM,\$LO\" >/dev/null 2>&1 || true
  echo \"PULSE_LOW n=\$n \$(date +%H:%M:%S) \$DOM,\$LO\" >>\"\$LOG\"
  sleep \"\$ON\"
  [ -f \"\$STOP\" ] && break
  mx-smi -i \"\$VR\" --set-dpm-max \"\$DOM,\$HI\" >/dev/null 2>&1 || true
  echo \"PULSE_HIGH n=\$n \$(date +%H:%M:%S) \$DOM,\$HI\" >>\"\$LOG\"
  sleep \"\$OFF\"
  n=\$((n+1))
done
mx-smi -i \"\$VR\" --set-dpm-max \"\$DOM,\$HI\" >/dev/null 2>&1 || true
echo \"PULSE_DONE n=\$n\" >>\"\$LOG\"
PSEOF
chmod +x /tmp/freq_pulse.sh
: > '$out/injection.log'
nohup bash /tmp/freq_pulse.sh '$out' $vr $dom $lo $hi $on_s $off_s >/dev/null 2>&1 & echo PULSE_PID=\$!
echo PULSE_ARMED domain=$dom low=$lo high=$hi on=${on_s}s off=${off_s}s" 2>/dev/null
  pexec "$MASTER" "printf '%s\n' \
    'SIDECAR_WARMUP kind=freq_pulse mid_after_measure_${INJECT_START_MEASURE_STEP}' \
    'SIDECAR_START kind=freq_pulse domain=${dom} level=${lo} restore=${hi} on_s=${on_s} off_s=${off_s} victim=${vr}' \
    'FREQ_PULSE_ARMED domain=${dom} low=${lo} high=${hi} on=${on_s} off=${off_s}' \
    >>'$out/injection.log'" 2>/dev/null || true
}
stop_freq_pulse() {
  local out="$1"
  pexec "$MASTER" "touch '$out/pulse.stop' 2>/dev/null; sleep 2; pkill -f '/tmp/freq_pulse.sh' 2>/dev/null || true; exit 0" 2>/dev/null || true
  freq_restore || true
}

fire_training() {   # $1=port $2=out_dir $3=detect_env $4=round
  local port="$1" out="$2" denv="$3" rnd="$4"
  echo "  fire_training begin port=$port out=$out" >&2
  for ((n=0; n<NNODES; n++)); do
    local launcher
...
LAUNCHER
    ...
    echo "  upload launcher n=$n pod=${PODS[$n]}" >&2
    printf '%s' "$launcher" | pexec_i "${PODS[$n]}" "cat > /tmp/run_${GROUP_ID}.sh && chmod +x /tmp/run_${GROUP_ID}.sh" 2>/dev/null || true
  done
  # 全并行发射（kubectl/pkill 类退出码在 set -e 下必须吞掉，否则只打出第一个 ok 就整案退出）
  echo "  parallel fire NNODES=$NNODES" >&2
  for ((n=0; n<NNODES; n++)); do
    pexec "${PODS[$n]}" "setsid nohup bash /tmp/run_${GROUP_ID}.sh </dev/null >/dev/null 2>&1 & echo ok; exit 0" 2>/dev/null &
  done
  wait || true
  echo "  fire_training end" >&2
  return 0
}
...
# ===== main loop =====
# P1-HW-C：freq / freq_pulse 均走间歇脉冲（区别于 1A 恒定 mid-set）
IS_FREQ=0
case "$INJECT_KIND" in
  freq|freq_pulse) IS_FREQ=1 ;;
esac
port=$BASE_PORT
pipe_rc=0
...
export XPU_TIMER_LAUNCH_SAMPLE=\${XPU_TIMER_LAUNCH_SAMPLE:-32};"
    ...
    if [ "$IS_FREQ" = "1" ] && [ "$inj" = "yes" ]; then
      # P1-HW-C / OUTLINE 1C：满档起训 → measure 窗间歇打低再恢复（tip/p99 叙事）
      freq_restore || true
      fire_training "$port" "$out" "$denv" "$r"
      echo "  fired(freq-pulse-armed domain=$FREQ_DOMAIN low=$FREQ_LEVEL high=$FREQ_RESTORE_LEVEL on=${PULSE_ON_S}s off=${PULSE_OFF_S}s)"
      wait_warmup "$out"
      if wait_measure_step "$out" "$INJECT_START_MEASURE_STEP"; then
        : > /dev/null
        pexec "$MASTER" "mkdir -p '$out'; : > '$out/injection.log'" 2>/dev/null || true
        start_freq_pulse "$out"
        echo "  freq PULSE armed domain=$FREQ_DOMAIN low=$FREQ_LEVEL on=${PULSE_ON_S}s off=${PULSE_OFF_S}s victim=$SIDECAR_LOCAL_RANK"
      else
        echo "  FAILED: freq pulse inject skipped (no start marker)"
        pipe_rc=1
      fi
    else
      if [ "$IS_FREQ" = "1" ]; then freq_restore || true; fi
      fire_training "$port" "$out" "$denv" "$r"; echo "  fired"
      wait_warmup "$out"
    fi

    # C2：注入窗内拉 Probing SQL
    ...
    # pulse：到 measure stop 停脉冲，再等训练收口
    if [ "$IS_FREQ" = "1" ] && [ "$inj" = "yes" ]; then
      (
        e=0
        while [ "$e" -lt 1800 ]; do
          if pexec "$MASTER" "test -f '$out/ranks/step_${INJECT_STOP_MEASURE_STEP}.marker'" 2>/dev/null; then
            echo "  pulse stop at measure step $INJECT_STOP_MEASURE_STEP"
            stop_freq_pulse "$out"
            exit 0
          fi
          if pexec "$MASTER" "ls '$out'/node_*.fail >/dev/null 2>&1" 2>/dev/null; then
            stop_freq_pulse "$out"; exit 0
          fi
          sleep 5; e=$((e+5))
        done
        stop_freq_pulse "$out"
    ...
    if wait_done "$out" 0; then
      echo "  COMPLETE"
    else
      echo "  FAILED"
      pipe_rc=1
    fi
    if [ -n "${PULSE_WATCH_PID:-}" ]; then
      wait "$PULSE_WATCH_PID" 2>/dev/null || true
      unset PULSE_WATCH_PID
    fi
    if [ "$IS_FREQ" = "1" ]; then
      stop_freq_pulse "$out" 2>/dev/null || freq_restore || true
    fi
    stop_sidecar
  done
done
