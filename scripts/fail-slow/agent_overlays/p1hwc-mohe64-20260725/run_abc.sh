...
PARENT="$(cd "$HERE/../.." && pwd)"
# overlay: shared scripts live in PARENT; pipeline may be PIPELINE_SH
...
case "$CASE_ID" in
  ...
  P3-EXT-B|9b)
    ...
    MODE="${MODE:-host_bound}"
    ACCEPT_MIN_RATIO="${ACCEPT_MIN_RATIO:-1.3}"
    # Loud IO 咬合：ckpt 风暴 + fio 同目录 + 训练盘读（fio 否则咬不到 step_ms）
    CKPT_EVERY="${CKPT_EVERY:-5}"
    FLUSH_EVERY="${FLUSH_EVERY:-1}"
    CKPT_DIR="${CKPT_DIR:-/workspace/probe-bundle/ckpt}"
    IO_PAYLOAD="${IO_PAYLOAD:-/workspace/probe-bundle/ckpt/io_payload.bin}"
    IO_READ_KB="${IO_READ_KB:-8192}"
    IO_STRESS_DIR="${IO_STRESS_DIR:-/workspace/probe-bundle/io_stress}"
    FIO_DIR="${FIO_DIR:-$CKPT_DIR}"
    FIO_FILE="${FIO_FILE:-$CKPT_DIR/fio_ckpt_storm.bin}"
    FIO_SIZE="${FIO_SIZE:-4G}"
    # attempt3+：主线程同步 IO，让盘争用进 step_ms（默认 2 会 prefetch 稀释）
    DL_WORKERS="${DL_WORKERS:-0}"
    FIO_IOENGINE="${FIO_IOENGINE:-libaio}"
    FIO_NUMJOBS="${FIO_NUMJOBS:-16}"
    FIO_IODEPTH="${FIO_IODEPTH:-64}"
    DROP_CACHES_BEFORE_INJECT="${DROP_CACHES_BEFORE_INJECT:-1}"
    STRESS_HDD="${STRESS_HDD:-1}"
    export CKPT_EVERY FLUSH_EVERY CKPT_DIR IO_PAYLOAD IO_READ_KB IO_STRESS_DIR
    export DL_WORKERS FIO_IOENGINE FIO_NUMJOBS FIO_IODEPTH
    export FIO_DIR FIO_FILE FIO_SIZE DROP_CACHES_BEFORE_INJECT STRESS_HDD
    ;;
  P3-EXT-C|9c)
  ...
  P3-SW-B|8b)
    ...
    ;;
  P1-HW-A|1a)
    # OUTLINE 1A 真·改频；无权限时改走 cube-proxy（另开 attempt）
    CASE="P1-HW-A"; INJECT_KIND="freq"
    INJECT_ARGS="${INJECT_ARGS:-level=4}"
    MODE="${MODE:-gpu_bound}"
    ACCEPT_MIN_RATIO="${ACCEPT_MIN_RATIO:-1.3}"
    ...
    ;;
  P1-HW-C|1c)
    # OUTLINE 1C 功耗墙间歇：脉冲 dpm；验收 tip/p99/max（勿只看中位）
    CASE="P1-HW-C"; INJECT_KIND="freq_pulse"
    INJECT_ARGS="${INJECT_ARGS:-level=0,pulse=1}"
    MODE="${MODE:-gpu_bound}"
    ACCEPT_MIN_RATIO="${ACCEPT_MIN_RATIO:-1.3}"
    FREQ_DOMAIN="${FREQ_DOMAIN:-xcore}"
    FREQ_LEVEL="${FREQ_LEVEL:-0}"
    FREQ_RESTORE_LEVEL="${FREQ_RESTORE_LEVEL:-9}"
    PULSE_ON_S="${PULSE_ON_S:-2}"
    PULSE_OFF_S="${PULSE_OFF_S:-8}"
    export FREQ_DOMAIN FREQ_LEVEL FREQ_RESTORE_LEVEL PULSE_ON_S PULSE_OFF_S
    ;;
  *) echo "unsupported CASE_ID=$CASE_ID (P1-HW-A/P1-HW-C/…)" >&2; exit 2 ;;
esac
...
run_config() {
  ...
  CASE="$CASE" INJECT_KIND="$INJECT_KIND" INJECT_ARGS="$INJECT_ARGS" \
    ...
    FIO_IOENGINE="${FIO_IOENGINE:-libaio}" \
    FIO_NUMJOBS="${FIO_NUMJOBS:-16}" FIO_IODEPTH="${FIO_IODEPTH:-64}" \
    FIO_DIR="${FIO_DIR:-}" FIO_FILE="${FIO_FILE:-}" FIO_SIZE="${FIO_SIZE:-4G}" \
    ...
    bash "${PIPELINE_SH:-$HERE/pipeline_freq_pulse.sh}"
}
...
pull_results
# 终态验收：P1-HW-C 用 tip/p99/max；其它走 median ratio
if [[ "$ACCEPT_SCRIPT" == *accept_p1hwc_tip* ]] || [[ "$ACCEPT_SCRIPT" == *accept_p1swc_spike* ]]; then
  python3 "$ACCEPT_SCRIPT" \
    --result-root "$LOCAL_RESULT_ROOT" \
    --case "$CASE" \
    --min-median-ratio "${ACCEPT_MIN_RATIO:-1.3}" \
    --min-p99-ratio "${ACCEPT_MIN_P99_RATIO:-1.5}" \
    --min-max-ratio "${ACCEPT_MIN_MAX_RATIO:-2.5}" \
    --victim-rank "${SIDECAR_LOCAL_RANK:-7}" \
    --write-md "$LOCAL_RESULT_ROOT/acceptance_${CASE}_tip.md" \
    || true
else
  python3 "$ACCEPT_SCRIPT" \
    --result-root "$LOCAL_RESULT_ROOT" \
