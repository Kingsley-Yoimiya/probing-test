...
# ===== 参数 =====
CASE="${CASE:?need CASE}"
INJECT_KIND="${INJECT_KIND:?need INJECT_KIND (cube|hbm|1b|1c|2ext|freq|stress_cpu|stress_vm|stress_page|stress_io|none)}"
...
# fio ioengine：默认 libaio（iodepth 才生效）；不可用时可 FIO_IOENGINE=psync 回退
...
DROP_CACHES_BEFORE_INJECT="${DROP_CACHES_BEFORE_INJECT:-0}"
STRESS_HDD="${STRESS_HDD:-0}"
...
if [ "${USE_VCCTL:-0}" = "1" ]; then
  ...
  pexec_i() { kubectl --request-timeout=60s --kubeconfig="$KC" -n "$NS" exec -i "$1" -- bash -c "$2"; }
...
MLOCK_N=$(echo "$INJECT_ARGS" | grep -oE 'mlock_n=[0-9]+' | cut -d= -f2 || true); MLOCK_N="${MLOCK_N:-${MLOCK_N_ENV:-0}}"
echo "  inject_parse DUTY=$DUTY SIZE=$SIZE FRAC=$FRAC CPU_LOAD=$CPU_LOAD CPU_N=${CPU_N:-} CPU_FRAC=${CPU_FRAC:-} VM_N=$VM_N VM_BYTES=$VM_BYTES MLOCK_N=$MLOCK_N DROP=${DROP_CACHES_BEFORE_INJECT:-0}"
...
start_sidecar() {   # 在 victim(node0)起注入; freq / 内联 8a 不走这里
  ...
  case "$INJECT_KIND" in
    ...
    stress_page)
      # P3-HW-A（OUTLINE 7A）：ECC/换页代理 — mid-window alloc+page-in（可选 mlock）；
      # ≠ P3-EXT-C（stress_vm 96×6G 带宽扫、证据常落 host_psi_cpu）：本 case 用 stress_page + drop_caches，
      #   证据优先 pgmajfault/PSI memory（mohe 宿主常 SwapTotal=0，swap 证据可能不可用，须如实记）
      # a4：更多 workers×较小 bytes（默认 64×6G）稳住可复现；mlock_n=0 跳过 --mlock（a1 OOM）
      # 注：集群 stress-ng 0.13.12 无 --vm-stride；持续触达靠 --vm-keep --page-in
      # 预检 MemAvailable；drop_caches 后起 vm；timeout 默认 900s 盖过 measure 窗
      local _drop="${DROP_CACHES_BEFORE_INJECT:-1}"
      local _mlock="${MLOCK_N:-0}"
      local _to="${STRESS_TIMEOUT_S:-900}"
      local _mlock_flag=""
      if [ "${_mlock}" != "0" ] && [ -n "${_mlock}" ]; then
        _mlock_flag="--mlock ${_mlock}"
      fi
      pexec "$v" ": >'$out/injection.log'; echo PRE_MEM=\$(awk '/MemTotal|MemAvailable|MemFree/{printf \"%s=%s \",\$1,\$2}' /proc/meminfo) >>'$out/injection.log'; echo PRE_PGMAJ=\$(awk '/pgmajfault/{print \$2}' /proc/vmstat) >>'$out/injection.log'; echo PRE_PSWPIN=\$(awk '/pswpin/{print \$2}' /proc/vmstat) >>'$out/injection.log'; echo PRE_PSI_MEM=\$(head -1 /proc/pressure/memory 2>/dev/null || echo UNAVAIL) >>'$out/injection.log'; echo PRE_SWAP=\$(awk '/SwapTotal|SwapFree/{printf \"%s=%s \",\$1,\$2}' /proc/meminfo) >>'$out/injection.log'; if [ '$_drop' = '1' ]; then (sync; echo 3 >/proc/sys/vm/drop_caches && echo DROP_CACHES=ok || echo DROP_CACHES=fail) >>'$out/injection.log' 2>&1 || echo DROP_CACHES=fail >>'$out/injection.log'; fi; echo SIDECAR_START stress_page vm_n=${VM_N}_bytes=${VM_BYTES}_mlock=${_mlock}_drop=${_drop}_to=${_to} >>'$out/injection.log'; nohup stress-ng --vm $VM_N --vm-bytes $VM_BYTES --vm-keep --page-in ${_mlock_flag} --timeout ${_to}s >>'$out/injection.log' 2>&1 & echo SC=\$!; ( sleep 25; echo POST25_PGMAJ=\$(awk '/pgmajfault/{print \$2}' /proc/vmstat); echo POST25_PSI_MEM=\$(head -1 /proc/pressure/memory 2>/dev/null || echo UNAVAIL); echo POST25_MEM=\$(awk '/MemAvailable|MemFree/{printf \"%s=%s \",\$1,\$2}' /proc/meminfo); echo POST25_SWAP=\$(awk '/SwapTotal|SwapFree/{printf \"%s=%s \",\$1,\$2}' /proc/meminfo); pgrep -c -x stress-ng || echo STRESS_ALIVE=0 ) >>'$out/injection.log' 2>&1 &" 2>/dev/null ;;
    stress_io)
      # Loud IO：默认 fio 打 CKPT_DIR 下大文件（与 checkpoint 风暴同目录争用）；
      # FIO_DIR / FIO_FILE / FIO_SIZE 可覆盖。可选 C1 前 drop_caches + stress-ng --hdd 辅注入。
      # FIO_IOENGINE 默认 libaio；若引擎不可用由外层探测后改 psync（仍计 attempt）。
      local _fio_dir="${FIO_DIR:-${CKPT_DIR:-/workspace/probe-bundle/ckpt}}"
      local _fio_file="${FIO_FILE:-$_fio_dir/fio_ckpt_storm.bin}"
      local _fio_size="${FIO_SIZE:-4G}"
      local _fio_tag="fio_loud_nj${FIO_NUMJOBS}_${FIO_IOENGINE}_direct_ckptdir"
      local _drop="${DROP_CACHES_BEFORE_INJECT:-1}"
      local _hdd="${STRESS_HDD:-0}"
      pexec "$v" "mkdir -p '$_fio_dir'; test -f '$_fio_file' || dd if=/dev/zero of='$_fio_file' bs=1M count=4096 status=none; : >'$out/injection.log'; if [ '$_drop' = '1' ]; then (echo 3 >/proc/sys/vm/drop_caches && echo DROP_CACHES=ok || echo DROP_CACHES=fail) >>'$out/injection.log' 2>&1 || echo DROP_CACHES=fail >>'$out/injection.log'; fi; printf '%s\n' 'SIDECAR_START ${_fio_tag} dir=$_fio_dir file=$_fio_file' >>'$out/injection.log'; nohup fio --name=io_stress --filename='$_fio_file' --ioengine=$FIO_IOENGINE --rw=randrw --bs=64k --size=$_fio_size --numjobs=$FIO_NUMJOBS --iodepth=$FIO_IODEPTH --direct=1 --overwrite=1 --time_based --runtime=600 --group_reporting >>'$out/injection.log' 2>&1 & echo SC=\$!; if [ '$_hdd' = '1' ]; then nohup stress-ng --hdd 8 --hdd-bytes 1G --temp-path '$_fio_dir' --timeout 600s >>'$out/injection.log' 2>&1 & echo HDD=\$!; echo SIDECAR_START stress_hdd_aux n=8 path=$_fio_dir >>'$out/injection.log'; fi; echo SIDECAR_START ${_fio_tag} dir=$_fio_dir file=$_fio_file" 2>/dev/null ;;
    8a|inline_8a|8b|inline_8b|none) : ;;  # 8a/8b 走训练进程 INLINE_INJECT
...
export INLINE_GC_STALL_S=${INLINE_GC_STALL_S:-0.25};"
    ...
    # P3-SW-B：进程内联 8b（外挂 sidecar 在大内存机咬空）→ h3c 冻结 MB=16 STALL=0.25
    if [ "$inj" = "yes" ] && { [ "$INJECT_KIND" = "8b" ] || [ "$INJECT_KIND" = "inline_8b" ]; }; then
      denv="${denv}
...
export INLINE_HBM_COPIES=${INLINE_HBM_COPIES:-48};"
    ...
    if [ "$IS_FREQ" = "1" ] && [ "$inj" = "yes" ]; then
      ...
      elif [ "$inj" = "yes" ] && { [ "$INJECT_KIND" = "8b" ] || [ "$INJECT_KIND" = "inline_8b" ]; }; then
        echo "  inline_8b armed (victim local_rank=$SIDECAR_LOCAL_RANK mb=${INLINE_8B_MB:-16} stall=${INLINE_8B_STALL_S:-0.25})"
        pexec "${PODS[0]}" "printf '%s\n' 'SIDECAR_WARMUP kind=inline_8b' 'SIDECAR_START kind=inline_8b' >'$out/injection.log'" 2>/dev/null || true
    ...
    # 内联 8a / 8b / inline hbm 无需外部 stop
    if [ "$inj" = "yes" ] && [ "$INJECT_KIND" != "8a" ] && [ "$INJECT_KIND" != "inline_8a" ] \
       && [ "$INJECT_KIND" != "8b" ] && [ "$INJECT_KIND" != "inline_8b" ] \
