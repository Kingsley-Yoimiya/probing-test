...
case "$CASE_ID" in
  ...
  P3-EXT-C|9c)
    ...
    ;;
  P3-HW-A|7a)
    # OUTLINE 7A：ECC/换页代理（真实 ECC 不可注入）
    # a4：更多 workers×较小 bytes（64×6G）+ drop_caches + page-in；mlock=0
    # ≠ P3-EXT-C：EXT-C 是 stress_vm 96×6G（带宽/PSI-cpu）；本 case 仍 stress_page + drop_caches，证据优先 pgmajfault
    CASE="P3-HW-A"; INJECT_KIND="stress_page"
    INJECT_ARGS="${INJECT_ARGS:-vm_n=64,vm_bytes=6G,mlock_n=0}"
    MODE="${MODE:-host_bound}"
    ACCEPT_MIN_RATIO="${ACCEPT_MIN_RATIO:-1.15}"
    DROP_CACHES_BEFORE_INJECT="${DROP_CACHES_BEFORE_INJECT:-1}"
    STRESS_TIMEOUT_S="${STRESS_TIMEOUT_S:-900}"
    export DROP_CACHES_BEFORE_INJECT STRESS_TIMEOUT_S
    ;;
  P3-SW-A|8a)
  ...
  P3-SW-B|8b)
    ...
    ACCEPT_MIN_RATIO="${ACCEPT_MIN_RATIO:-1.3}"
    ;;
  *) echo "unsupported CASE_ID=$CASE_ID (P1-EXT-A/B, P3-EXT-A/B/C, P3-HW-A, P3-SW-A/B)" >&2; exit 2 ;;
esac
...
run_config() {
  ...
  CASE="$CASE" INJECT_KIND="$INJECT_KIND" INJECT_ARGS="$INJECT_ARGS" \
    ...
    bash "${PIPELINE_SH:-$HERE/pipeline_page.sh}"
}
