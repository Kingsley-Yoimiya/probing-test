#!/usr/bin/env bash
# pipeline_cpufreq_mid.sh — P3-HW-B / OUTLINE 7B：主机 CPU 温度墙（中途锁低 cpufreq）
#
# ≠ P1-HW-A（GPU xcore）≠ P3-HW-A（换页）≠ P3-EXT-C（stress_vm）
# 真路径：victim 宿主 /sys/.../cpufreq/scaling_max_freq 中途压低；需 root（已验证可写）
# host_bound step；measure start 后注入，结束恢复
#
# 用法(env 驱动):
...
# ===== 参数 =====
CASE="${CASE:?need CASE}"
INJECT_KIND="${INJECT_KIND:?need INJECT_KIND (cube|hbm|1b|1c|2ext|freq|cpufreq|stress_cpu|stress_vm|stress_io|none)}"
...
SEQ="${SEQ:-1024}"
BATCH="${BATCH:-8}"
# CPU 温墙：kHz；可用档 2700000/1600000/1200000；Loud 默认锁最低
CPU_FREQ_KHZ="${CPU_FREQ_KHZ:-1200000}"
CPU_FREQ_RESTORE_KHZ="${CPU_FREQ_RESTORE_KHZ:-2700000}"
FREQ_LEVEL="${FREQ_LEVEL:-4}"          # 保留兼容（GPU 分支不用）
...
# P3-HW-B：khz= from INJECT_ARGS overrides CPU_FREQ_KHZ
_khz=$(echo "$INJECT_ARGS" | grep -oE 'khz=[0-9]+' | cut -d= -f2 || true)
[ -n "$_khz" ] && CPU_FREQ_KHZ="$_khz"
echo "  inject_parse DUTY=$DUTY SIZE=$SIZE FRAC=$FRAC CPU_LOAD=$CPU_LOAD CPU_N=${CPU_N:-} CPU_FRAC=${CPU_FRAC:-} VM_N=$VM_N VM_BYTES=$VM_BYTES CPU_FREQ_KHZ=$CPU_FREQ_KHZ"
...
# CPU cpufreq：victim 宿主全核 scaling_max_freq（OUTLINE 7B）；需 root（pod CapEff 全开已验证）
cpufreq_set() {   # $1=kHz
  local khz="$1"
  pexec "$MASTER" "
    set +e
    ...
    for d in /sys/devices/system/cpu/cpu[0-9]*/cpufreq; do
      [ -w \"\$d/scaling_max_freq\" ] || { fail=\$((fail+1)); continue; }
      if echo '$khz' > \"\$d/scaling_max_freq\" 2>/dev/null; then ok=\$((ok+1)); else fail=\$((fail+1)); fi
    done
    cur=\$(cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_cur_freq 2>/dev/null || echo NA)
    max0=\$(cat /sys/devices/system/cpu/cpu0/cpufreq/scaling_max_freq 2>/dev/null || echo NA)
    echo CPUFREQ_SET_khz=${khz}_ok=\${ok}_fail=\${fail}_cur0=\${cur}_max0=\${max0}
  " 2>/dev/null
}
cpufreq_restore() {
  local khz="${CPU_FREQ_RESTORE_KHZ:-2700000}"
  pexec "$MASTER" "
    set +e
    mount -o remount,rw /sys 2>/dev/null || true
    ok=0
    for d in /sys/devices/system/cpu/cpu[0-9]*/cpufreq; do
      ...
      if echo '$khz' > \"\$d/scaling_max_freq\" 2>/dev/null; then ok=\$((ok+1)); fi
    done
    echo CPUFREQ_RESTORED_khz=${khz}_ok=\${ok}
  " 2>/dev/null || true
}
# 兼容旧名（本 overlay 只走 CPU）
freq_set() { cpufreq_set "${CPU_FREQ_KHZ}"; }
freq_restore() { cpufreq_restore; }
...
# ===== main loop =====
# OUTLINE 7B：freq|cpufreq = 主机 CPU 温墙改频（非 GPU xcore）
IS_FREQ=0
case "$INJECT_KIND" in freq|cpufreq) IS_FREQ=1 ;; esac
port=$BASE_PORT
pipe_rc=0
...
export INLINE_HBM_COPIES=${INLINE_HBM_COPIES:-48};"
    ...
    if [ "$IS_FREQ" = "1" ] && [ "$inj" = "yes" ]; then
      # P3-HW-B / OUTLINE 7B：先满频起训，measure start 后中途锁低 CPU scaling_max_freq
      cpufreq_restore || true
      fire_training "$port" "$out" "$denv" "$r"; echo "  fired(cpufreq-mid-armed khz=$CPU_FREQ_KHZ)"
      wait_warmup "$out"
      if wait_measure_step "$out" "$INJECT_START_MEASURE_STEP"; then
        cpufreq_set "$CPU_FREQ_KHZ"
        pexec "$MASTER" "printf '%s\n' \
          'SIDECAR_WARMUP kind=cpufreq mid_after_measure_${INJECT_START_MEASURE_STEP}' \
          'SIDECAR_START kind=cpufreq khz=${CPU_FREQ_KHZ} host=victim_master' \
          'CPUFREQ_MID_SET khz=${CPU_FREQ_KHZ}' \
          >'$out/injection.log'" 2>/dev/null || true
        echo "  cpufreq MID-SET khz=$CPU_FREQ_KHZ (victim host)"
      else
        echo "  FAILED: cpufreq mid inject skipped (no start marker)"
        pipe_rc=1
      fi
    else
      # 统一从训练中触发。WARMUP=50 时 measure 100/300 即总 step 150--350；
      # cube/hbm 自身再预热 >=5 秒，避免 MetaX 时间片隔离导致的伪阴性。
      if [ "$IS_FREQ" = "1" ]; then cpufreq_restore || true; fi
    ...
    # 内联 8a / inline hbm / cpufreq 无需外部 stop（cpufreq 在 wait_done 后 restore）
    if [ "$inj" = "yes" ] && [ "$INJECT_KIND" != "8a" ] && [ "$INJECT_KIND" != "inline_8a" ] \
       && [ "$INJECT_KIND" != "freq" ] && [ "$INJECT_KIND" != "cpufreq" ] \
    ...
    stop_sidecar
    if [ "$IS_FREQ" = "1" ]; then cpufreq_restore; fi
  done
done
