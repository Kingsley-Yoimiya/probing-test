...
case "$CASE_ID" in
  ...
  P1-HW-A|1a)
    ...
    ;;
  P3-HW-B|7b)
    # OUTLINE 7B：主机 CPU 温度墙降频（真·cpufreq；≠ campaign 误写的 stress_vm）
    CASE="P3-HW-B"; INJECT_KIND="cpufreq"
    INJECT_ARGS="${INJECT_ARGS:-khz=1200000}"
    MODE="${MODE:-host_bound}"
    ACCEPT_MIN_RATIO="${ACCEPT_MIN_RATIO:-1.15}"
    CPU_FREQ_KHZ="${CPU_FREQ_KHZ:-1200000}"
    CPU_FREQ_RESTORE_KHZ="${CPU_FREQ_RESTORE_KHZ:-2700000}"
    export CPU_FREQ_KHZ CPU_FREQ_RESTORE_KHZ
    ;;
  *) echo "unsupported CASE_ID=$CASE_ID (P3-HW-B primary)" >&2; exit 2 ;;
esac
...
run_config() {
  ...
  CASE="$CASE" INJECT_KIND="$INJECT_KIND" INJECT_ARGS="$INJECT_ARGS" \
    ...
    bash "${PIPELINE_SH:-$HERE/pipeline_cpufreq_mid.sh}"
}
