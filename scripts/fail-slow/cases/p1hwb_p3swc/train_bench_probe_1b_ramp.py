...
def main() -> None:
    ...
    # Isolated P1-HW-B ramp: INLINE_INJECT=hbm|hbm_ramp；INLINE_HBM_RAMP=1 → copies 线性 1→max
    do_inline_hbm = (
        inline in ("hbm", "hbm_ramp")
        and local == inline_victim
        and node_rank == 0
    )
    # 本文件仅用于 P1-HW-B 隔离战役：同步为 train_bench_probe.py 后，hbm 一律渐进 ramp
    hbm_ramp = True
    leak_buf: list = []
    hbm_bufs = None
    ...
    if do_inline_hbm:
        hbm_mb = max(32, min(int(os.environ.get("INLINE_HBM_MB", "512")), 1024))
        ...
        print(
            ...
            f"ramp={hbm_ramp} copies_max={hbm_copies_max}",
            flush=True,
        )
    ...
    try:
        for i in range(args.iters):
            ...
            # 1B 渐进：窗内 step 从 copies → copies_max（对齐 OUTLINE 剂量旋钮）
            cur_hbm_copies = 0
            if in_win_hbm:
                if hbm_ramp:
                    span = max(1, inline_stop - inline_start - 1)
                    frac = (i - inline_start) / span
                    cur_hbm_copies = max(
                        1,
