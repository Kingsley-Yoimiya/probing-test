#!/usr/bin/env bash
# self-timeout wrapper body begins after re-exec guard
if [ "${_DUMP_INNER:-0}" != "1" ]; then
  export _DUMP_INNER=1
  LIMIT="${DUMP_TIMEOUT_S:-90}"
  if command -v timeout >/dev/null 2>&1; then
    timeout "$LIMIT" env _DUMP_INNER=1 bash "$0" "$@"
    rc=$?
    [ "$rc" -eq 124 ] && echo "dump_probing_sql TIMEOUT after ${LIMIT}s" >&2
    exit 0
  fi
  bash "$0" "$@" &
  spid=$!
  ( sleep "$LIMIT"; kill -9 "$spid" 2>/dev/null || true ) &
