...
start_sidecar() {   # 在 victim(node0)起注入; freq / 内联 8a 不走这里
  ...
  case "$INJECT_KIND" in
    ...
    8a|inline_8a|2a|inline_2a|5c|topo|none) : ;;  # 内联 / 拓扑 env 不走 sidecar
...
for r in $(seq 1 "$ROUNDS"); do
  ...
  for cfg in "${CONFIGS[@]}"; do
    ...
    clean_group
    # LOCAL_FS：必须清掉各 pod 上本 config 旧 done/fail，否则 wait_done 会吃到残留 marker
    for ((n=0; n<NNODES; n++)); do
...
export INLINE_GC_STALL_S=${INLINE_GC_STALL_S:-0.25};"
    ...
    # P1-SW-A：显存碎片化→骤停（隔离副本 train_bench_probe_2a.py）
    if [ "$inj" = "yes" ] && { [ "$INJECT_KIND" = "2a" ] || [ "$INJECT_KIND" = "inline_2a" ]; }; then
      denv="${denv}
...
export INLINE_HBM_COPIES=${INLINE_HBM_COPIES:-48};"
    ...
    if [ "$IS_FREQ" = "1" ] && [ "$inj" = "yes" ]; then
      ...
      elif [ "$inj" = "yes" ] && [ "$INJECT_KIND" != "none" ] \
        && [ "$INJECT_KIND" != "8a" ] && [ "$INJECT_KIND" != "inline_8a" ] \
        && [ "$INJECT_KIND" != "2a" ] && [ "$INJECT_KIND" != "inline_2a" ] \
        && [ "$INJECT_KIND" != "5c" ] && [ "$INJECT_KIND" != "topo" ]; then
      ...
      elif [ "$inj" = "yes" ] && { [ "$INJECT_KIND" = "2a" ] || [ "$INJECT_KIND" = "inline_2a" ]; }; then
        echo "  inline_2a armed (victim local_rank=$SIDECAR_LOCAL_RANK chunks=${INLINE_2A_CHUNKS:-8})"
        pexec "${PODS[0]}" "printf '%s\n' 'SIDECAR_WARMUP kind=inline_2a' 'SIDECAR_START kind=inline_2a' >'$out/injection.log'" 2>/dev/null || true
      elif [ "$inj" = "yes" ] && { [ "$INJECT_KIND" = "5c" ] || [ "$INJECT_KIND" = "topo" ]; }; then
        echo "  topo_5c armed (bad HCA order / device map)"
      fi
    fi
    ...
    # 内联 8a/2a / topo / inline hbm 无需外部 stop
    if [ "$inj" = "yes" ] && [ "$INJECT_KIND" != "8a" ] && [ "$INJECT_KIND" != "inline_8a" ] \
       && [ "$INJECT_KIND" != "2a" ] && [ "$INJECT_KIND" != "inline_2a" ] \
       && [ "$INJECT_KIND" != "5c" ] && [ "$INJECT_KIND" != "topo" ] \
