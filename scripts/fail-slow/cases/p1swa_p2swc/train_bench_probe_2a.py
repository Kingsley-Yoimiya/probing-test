...
def main() -> None:
    ...
    def step_instrumented(
        ...
        """返回 (data_ms, compute_ms, comm_ms, wait_ms, step_ms, loss, frag_stall_ms)。"""
        ...
        # P1-SW-A：碎片累积 + 骤停必须在 barrier 之前，才能拖全局 step_ms
        if frag_action is not None:
            ...
            if do_stall and stall_mb > 0:
                ...
                # Loud 地板：真实碎片整理若太快，补齐最小 stall，否则同步点咬不动全局中位
        ...
        if decompose:
            ...
            data_ms = (t_data - t0) * 1e3
        step_ms = (t1 - t0) * 1e3
        return data_ms, compute_ms, comm_ms, wait_ms, step_ms, float(loss.detach().cpu()), frag_stall_ms

    # ── Warmup(同样的训练逻辑, 建立 CUDA context / autotune) ──
    for _ in range(args.warmup):
        step_instrumented()  # 忽略多余返回值
    dist.barrier()
    ...
    # P1-SW-A（本隔离副本）：显存碎片化→骤停（INLINE_INJECT=2a）。
    ...
    leak_buf: list = []
    # 2a：交错尺寸块；偶删奇数索引 → 碎片；剂量=每步新增块数 / 骤停大块 MB
    frag_live: list = []
    ...
    # Loud 地板：与 8a 类似，保证 barrier 前有足够延迟拖全局中位
    frag_min_stall_s = float(os.environ.get("INLINE_2A_STALL_S", "0.25"))
    frag_sizes = [256, 1024, 4096, 16384, 65536, 262144, 524288, 1048576]  # elements float16
    hbm_bufs = None
    ...
    try:
        for i in range(args.iters):
            ...
            # 2a：窗内每步碎片；窗后半段 barrier 前大块+最小 stall → 全局 step 抬升
            frag_action = None
            if in_win_2a:
            ...
            data_ms, compute_ms, comm_ms, wait_ms, step_ms, loss, frag_stall_ms = step_instrumented(
                force_gc=force_gc,
            ...
            rec = {
                ...
                "cuda_alloc_bytes": mem_alloc,
                "cuda_reserved_bytes": mem_reserved,
