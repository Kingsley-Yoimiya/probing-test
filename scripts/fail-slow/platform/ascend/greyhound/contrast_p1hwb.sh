...
inject_args: "inline_hbm_mb=${HBM_MB},inline_hbm_copies=${HBM_COPIES},inline_hbm_copies_max=${HBM_COPIES_MAX},ramp=${HBM_RAMP}"
...
run_arm() {
  ...
  if [[ "$do_inject" == "1" ]]; then
    ...
    export INLINE_HBM_COPIES_MAX="$HBM_COPIES_MAX"
    export INLINE_HBM_RAMP="$HBM_RAMP"
  fi
  ...
  if [[ "$do_inject" == "1" ]]; then
    ...
    while [[ $e -lt 2400 ]]; do
      if [[ -f "$out/ranks/step_${INJECT_START}.marker" ]]; then
        echo "  measure step ${INJECT_START} (${e}s) — inline hbm ramp active"
        break
      fi
    ...
    echo "SIDECAR_START kind=inline_hbm_ramp mb=${HBM_MB} copies=${HBM_COPIES} copies_max=${HBM_COPIES_MAX} ramp=${HBM_RAMP} victim=${VICTIM_LOCAL}" >>"$out/injection.log"
