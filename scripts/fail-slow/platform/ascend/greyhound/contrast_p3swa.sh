...
inject_args: "inline_gc_every=${GC_EVERY},inline_gc_stall_s=${GC_STALL_S}"
...
script: platform/ascend/greyhound/contrast_p3swa.sh
...
run_arm() {
  ...
  if [[ "$do_inject" == "1" ]]; then
    ...
    export INLINE_GC_EVERY="$GC_EVERY"
    export INLINE_GC_STALL_S="$GC_STALL_S"
  fi

  echo "========== $arm port=$port inject=$do_inject 8a every=${GC_EVERY} stall=${GC_STALL_S}s =========="
  ...
  if [[ "$do_inject" == "1" ]]; then
    ...
    while [[ $e -lt 2400 ]]; do
      if [[ -f "$out/ranks/step_${INJECT_START}.marker" ]]; then
        echo "  measure step ${INJECT_START} (${e}s) — inline 8a GC active"
        break
      fi
    ...
    grep -E "INLINE_8A|INLINE_GC|SIDECAR" "$out/node_0.log" 2>/dev/null | head -20 >"$out/injection.log" || true
    echo "SIDECAR_START kind=inline_8a every=${GC_EVERY} stall_s=${GC_STALL_S} victim=${VICTIM_LOCAL}" >>"$out/injection.log"
  fi
...
DOSE_DESC="INLINE 8a gc_every=${GC_EVERY} stall_s=${GC_STALL_S} victim=${VICTIM_LOCAL}; window [${INJECT_START},${INJECT_STOP}]"
