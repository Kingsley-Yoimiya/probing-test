#!/usr/bin/env python3
"""P1-SW-C tip/max dose_check appendix for XPUTimer contrast.
...
does not change detect_ok (XPUTimer rule path stays autonomous OR cross-run).
"""
...
def main() -> int:
    ...
    tip_block = {
        ...
        "note": f"P1-SW-C tip/max gate; median often blind; Probing gold tip max≈{args.gold_tip_max}",
        "window": [args.window_start, args.window_stop],
    }

    sp = Path(args.summary)
    meta = json.loads(sp.read_text(encoding="utf-8"))
    # Keep original dose_check (all-rank median); add tip_dose_check alongside.
    meta["tip_dose_check"] = tip_block
    # If median dose_check failed but tip passed, annotate overall dose honesty.
    dc = meta.get("dose_check") or {}
    meta["dose_check_combined"] = {
        "step_ms_median_pass": bool(dc.get("pass")),
        "tip_max_pass": bool(tip_pass),
        "pass": bool(dc.get("pass") or tip_pass),
        "preferred": "tip_max" if tip_pass and not dc.get("pass") else "step_ms_median",
    }
    ...
    appendix = [
        ...
        f"(med≥{args.accept_min_med_ratio} OR p99≥{args.accept_min_p99_ratio} OR max≥{args.accept_min_max_ratio})",
