#!/usr/bin/env python3
"""train_bench_swb.py — SW-B 隔离副本（fork of train_bench_probe.py）。

相对 probe 版增量:
  - INLINE_INJECT=2b / rare_shape：victim 在注入窗内改用罕见 seq（pad/truncate），
    jsonl 记 shape_seq（P1-SW-B / OUTLINE 2B）。
  - 并行：默认 DDP（PARALLEL=ddp）；124M @ 64-rank 足够。USE_FSDP=1 暂忽略
    （共享词表 Embedding 与 FSDP wrap 风险高，见 swb/README.md）。
  - 保留 INLINE_INJECT=8a / hbm 路径，勿与其它 agent 的 probe 主线交叉改动。
...
def _activate_probing_if_requested() -> None:
    ...
    try:
        ...
        print(f"[train_bench_swb] probing site_hook failed: {exc}", flush=True)
...
def main() -> None:
    ...
    # DDP only：USE_FSDP=1 低风险路径暂不启用（共享 Embedding + 动态 seq）
    if os.environ.get("USE_FSDP", "0").strip() in ("1", "true", "yes") and world >= 64:
        print("[train_bench_swb] USE_FSDP=1 ignored; staying on DDP", flush=True)
    ...
    def step_instrumented(
        ...
        """返回 (data_ms, compute_ms, comm_ms, wait_ms, step_ms, loss, shape_seq)。"""
        ...
        if seq_override is not None and int(seq_override) != int(idx.shape[1]):
            ...
            if idx.shape[1] > tgt:
                ...
                pad = torch.randint(
                    0, vocab, (idx.shape[0], tgt - idx.shape[1]),
        ...
        # 注意：DDP allreduce 发生在 opt.step() 内，e_m1 必须在 step 之后
        opt.step()
        # P2-SW-B：可选大 AllReduce 负载（C0/C1 同开），使通道钳制成为可测自变量
        stress_mb = int(os.environ.get("MCCL_STRESS_MB", "0") or "0")
        if stress_mb > 0:
        ...
        if decompose:
            ...
            data_ms = (t_data - t0) * 1e3
        step_ms = (t1 - t0) * 1e3
        return data_ms, compute_ms, comm_ms, wait_ms, step_ms, float(loss.detach().cpu()), shape_seq
    ...
    # ── Measure: 流式写本地盘(每步 append, 写在计时区之外, 每 flush_every 步 fsync) ──
    ...
    ckpt_dir = Path(os.environ.get("CKPT_DIR", "/workspace/probe-bundle/swb/ckpt"))
        ...
        for i in range(args.iters):
            ...
            if in_win_2b:
                ...
                if rare_frac is not None:
                    # 确定性分数：窗内前 frac 比例步用 rare（避免随机破坏复现）
            ...
            data_ms, compute_ms, comm_ms, wait_ms, step_ms, loss, shape_seq = step_instrumented(
                force_gc=force_gc,
